...
import { certificates } from '../actions/admin.actions';
...
const Certificates = () => {
  ...
  const [searchPlaceholder, setSearchPlaceholder] = useState('Certificate');
  const [searchBy, setSearchBy] = useState('certificates');
  ...
  useEffect(() => {
    ...
    if (Object.keys(userid).length === 0) {
      ...
      certificates(userid.EMSSAN).then((res) => {
  ...
  const gosubmit = (e) => {
    ...
    setFirstLoading(true);
    if (searchBy == 'certificates') {
      const filteredData = allattendataextac.filter((item) =>
        item.CTID.toLowerCase().includes(searchValue.toLowerCase())
      );
    ...
    if (searchBy == 'type') {
      const filteredData = allattendataextac.filter((item) =>
        item.CTTYPE.toLowerCase().includes(searchValue.toLowerCase())
      );
      const totalPages = Math.ceil(filteredData.length / perPage);
      setTotalPage(totalPages);
      const startIndex = (1 - 1) * perPage;
    ...
    if (searchBy == 'level') {
      const filteredData = allattendataextac.filter((item) =>
        item.CTLEVL.toLowerCase().includes(searchValue.toLowerCase())
      );
      const totalPages = Math.ceil(filteredData.length / perPage);
      setTotalPage(totalPages);
      const startIndex = (1 - 1) * perPage;
  ...
  const handleClearFilter = () => {
    setSearchPlaceholder('Certificate');
    setSearchBy('certificates');
    setSearchValue('');
    setFirstLoading(true);
